...
A _Dockerfile_ is a plain text file that says how to build the container. A _Docker image_ is what you get when you build a _Dockerfile_. A _Docker container_ is a running instance of a _Docker image_. You can run multiple containers from the same image. A _Docker Hub_ is a public regitry where _Docker images_ are stored and shared. _Docker volumes_ are storage areas that exist outside the container and persist.
