...
- **Credential** — *proves* it. Password, token, private key. Secret.
...
|---|---|---|---|
| **log in (don't stay logged in)** | a credential the server can recognize; stored in client memory | ***1 token*** <br> JWT <br>--- *or* <br> opaque session id | log in again every time you open the app |
| **+ stay logged in** | + store it on the device | 1 token — JWT *or* opaque session id | a long-lived credential, no way to kill it early |
| **+ log out everywhere** *(session id)* | logic to delete session id from DB | 1 token — opaque random string, looked up in a table | a DB lookup on **every** request |
| **+ log out everywhere** *(JWT)* | + a second, revocable token | 2 tokens — a JWT access token, plus one opaque refresh token **per device** | tech complexity: a table, an extra endpoint, client retry logic |
...
| **format** | usually a JWT — self-contained, so no DB lookup | usually opaque + random — needs a DB lookup, which is *what makes it revocable* | random string the server printed into your page |
...
both part of row 3. A CSRF token is a different axis entirely: it proves *where
a request came from*, not who you are, and it only exists in cookie-based apps,
...
- Piece 3 is a number the server calculated *from* pieces 1 and 2 plus its secret.
...
The server knows the user id because it *put it there* at login:
...
APIs because cookies are a *browser* feature — curl, mobile apps, and other
...
could only be obtained by *reading*. Server prints a random value into the page
...
produce the value. Not a secret from the user — a secret from other *sites*.
...
no CSRF. That's *why* we don't need a CSRF token.
...
the *response body*, which the attacker can't read. They caused a token to be
